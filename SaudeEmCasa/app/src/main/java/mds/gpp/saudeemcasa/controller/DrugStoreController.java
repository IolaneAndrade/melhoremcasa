...
public class DrugStoreController {
    ...
    public static float[] getDistance(Context context,ArrayList<DrugStore> list) {
        ...
        if(gps.canGetLocation()) {
            ...
            for (int i = 0; i < list.size(); i++) {
                ...
                Location drugStoreLocation = null;
                Double drugStoreLatitude = null;
                Double drugStoreLongitude = null;
                drugStoreLatitude.parseDouble(auxLatitude);
                drugStoreLongitude.parseDouble(auxLongitude);
                drugStoreLocation.setLatitude(drugStoreLatitude);
                drugStoreLocation.setLongitude(drugStoreLongitude);
                results[i] = gps.getLocation().distanceTo(drugStoreLocation);
            }
            return results;
        }else {
            return null;
